# scripts/run_sharegpt_prefix_single.py
...
def main():
    # Tag run as "single" for our metrics
    os.environ["PREFIX_MODE"] = "single"
    ...
    # Feed only single-turn prompts
    sim.run(mode="single-turn", send_fn=send_to_engine)

    # No need to run engine.step() for metrics
    engine.dump_prefix_metrics("/tmp/prefix_metrics_sharegpt_single")
    print("Single-turn metrics dumped to /tmp/prefix_metrics_sharegpt_single_*")
